...
contains
  ...
  subroutine NOP
    implicit none
    integer(kind=ik):: eleN, locN, eleNp, NNXS
    ...
    NNR1278 = ( 2*(NEL+NES) )* 2*NEL + 2*NEV*(2*NES+1)   !number of nodes in region 1&2, excluding the sharing line with region 3&4&5
    NNR1278p = NNR1278 + 2*NEL + 2*(NEL+NES)+1    !number of nodes in region 1&2, including the sharing line with region 3&4&5
    NNX34 = 4*NEL + 1
    NNX3456 = NNX34 + 2*NES
    NNXS = 2*NES+1
    !NNXV = 2*NEV + 1
...
200 format(A,i4)
    ...
    write(10,200) 'rad_ele =', NEL*2+NES
    close(10)


    allocate( globalNM(NTE,9), rowNM(NTE), columnNM(NTE), WFLAG(NTE), RegN(0:NTE), VE(NTE) )
    RegN(0) = 0
    !VE: vapor element
...
do eleN = 1, NTE
   ...
   if(eleN.le.NES*NEV) then  !region 8
      RegN(eleN) = 8
      rowNM(eleN) = (eleN-1)/NES + 1
      columnNM(eleN) = mod(eleN-1, NES) + 1
      
      !calculate globalN
      globalNb = 2*NNXS*(rowNM(eleN)-1) + 2*(columnNM(eleN)-1)
      do locN = 1, 9
         select case (locN)
         case(1)
            globalNM(eleN,locN) = globalNb + 2*NNXS + 1
         case(2)
            globalNM(eleN,locN) = globalNb + NNXS + 1
         case(3)
            globalNM(eleN,locN) = globalNb + 1
         case(4)
            globalNM(eleN,locN) = globalNb + 2*NNXS + 2
         case(5)
            globalNM(eleN,locN) = globalNb + NNXS + 2
         case(6)
            globalNM(eleN,locN) = globalNb + 2
         case(7)
            globalNM(eleN,locN) = globalNb + 2*NNXS + 3
         case(8)
            globalNM(eleN,locN) = globalNb + NNXS + 3
         case(9)
            globalNM(eleN,locN) = globalNb + 3
         end select

         Ngrid( globalNM(eleN,locN), 1 ) = 2*rowNM(eleN) - mod(locN-1,3) + 1
         Ngrid( globalNM(eleN,locN), 2 ) = 2*columnNM(eleN) + (locN-1)/3 - 1
      end do
      

   else if ( eleN.le.(NEL+NES)*NEL + NES*NEV ) then    !the element is in region 1,7
      !eleN = eleN - NES*NEV

      !calculate rowN & columnN
      do i = 1, NEL        !solve for the elements in region1&7 round by round
         if ( eleN.le.(i**2 + i*NES + NES*NEV) ) exit
      end do
      ! if(NEV.ge.NES) then
      ...
      ! else
         rowNM(eleN) = i
         columnNM(eleN) = i+NES
         if( eleN .lt. ( i**2 + i*NES + NES*NEV - i+1 ) ) then
            columnNM(eleN) = columnNM(eleN) - ( ( i**2 + i*NES + NES*NEV - i+1 ) - eleN )
         else if( eleN .gt. ( i**2 + i*NES + NES*NEV - i+1 ) ) then
            rowNM(eleN) = rowNM(eleN) - ( eleN - ( i**2 + i*NES + NES*NEV - i+1 ) )
         end if
      ! end if
      !      !  exit
      ! !    end if
      ...
      do locN = 1, 9
         ...
         i = max( NcolumnN-2*NES, NrowN )
         globalNM(eleN, locN) = i*(i+2*NES) - i+1 + 2*NEV*(2*NES+1)
         ! if(NEV.ge.NES) then
         ...
         !    end if
         ! else!???
            if( (NcolumnN-2*NES) .lt. NrowN ) then
...
if(eleN.eq.13 .and. locN.eq.9) write(*,*) globalNM(eleN, locN)
      ...
      if( columnNM(eleN).le.NES ) then   ! region 7
         RegN(eleN) = 7
      else   !region 8
         RegN(eleN) = 1
      end if
   ...
   else   !in region 3, 4 or 6
   
      eleNp = eleN - (NEL+NES)*NEL - NES*NEV

      !calculate rowN & columnN
      rowNM(eleN) = (eleNp-1) /(2*NEL+NES) + 1          !no dble
      columnNM(eleN) = mod( eleNp-1,(2*NEL+NES) ) + 1
   ...
   !VN&VE
   !vapor
   if( RegN(eleN).eq.8 .and. columnNM(eleN).eq.NES) then
      do locN = 7, 9
         VN( globalNM(eleN, locN) ) = 1
         if( globalNM(eleN, locN).eq. ( (2*NES+1)*(2*NEV+1) ) )  VN( globalNM(eleN, locN) ) = 0
      end do
   end if
...
do i = 1, NTE
   ...
   !free
   if( (RegN(i).eq.1 .and. rowNM(i).eq.1).or. (RegN(i).eq.4 .and. columnNM(i).eq.2*NEL+NES) ) then
      BCflagE(i,3) = 1
   end if
   ...
   ! end if
   !outer circle
   if( RegN(i).eq.8 .and. rowNM(i).eq.1 ) then
      BCflagE(i,4) = 1
   end if
   !substrate base
...
do i = 1, NTE
   ...
   else if( RegN(i).eq.8 .and. columnNM(i).eq.NES .and. ( j.eq.7 .or. j.eq.8 .or. j.eq.9 ) ) then
      BCflagN( globalNM(i,j), 2 ) = 1
   ...
   !free
   if( ( ( RegN(i).eq.4 .and. columnNM(i).eq.NES+2*NEL ).or.( RegN(i).eq.1 .and. rowNM(i).eq.1 ) ) &
        .and. ( j.eq.3 .or. j.eq.6 .or. j.eq.9 ) ) then
      BCflagN( globalNM(i,j), 3) = 1
      if( i.eq.NES*(NEV+1)+1 .and. j.eq.3 )  BCflagN( globalNM(i,j), 3) = 3
   else if( RegN(i).eq.8 .and. rowNM(i).eq.NEV .and. ( j.eq.1 .or. j.eq.4 .or. j.eq.7 ) ) then
      if( i.eq.NES*NEV .and. j.eq.7 ) cycle
   ...
   if( (RegN(i).eq.7 .or. RegN(i).eq.8) .and. columnNM(i).eq.1 .and. ( j.eq.1 .or. j.eq.2 .or. j.eq.3 ) ) then
      if(RegN(i).eq.7 .and. rowNM(i).eq.NEL .and. j.eq.1) cycle
...
do i = 1, NTE
   ...
   else if(RegN(i).eq.7) then
      if( rowNM(i).eq.1 ) then
         do j = 3,9,3
            algeS(globalNM(i,j)) = 3
         end do
      end if
   !    do j = 1, 9
   !       do n = 1, vlayer-1
...
do eleN = 1, NTE
   ...
   if(RegN(elen).eq.7) rowNM(eleN) = -rowNM(eleN) + NEM+NEL+1
   if(RegN(elen).eq.8) rowNM(eleN) = -rowNM(eleN) + NEM+NEL+NEV+1
end do



! !write all element and node information
...
! pause
  ...
  real(kind=rk) function damfac(x)
    ...
    if(x.gt.70.0_rk) then
       damfac = 0.05_rk

    else if(x.gt.5.0_rk) then
       damfac = 0.1_rk
