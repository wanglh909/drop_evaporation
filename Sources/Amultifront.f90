...
module front_mod
  use kind, only: rk, ik
  use omp_lib
  use data, only: NV=>NVar, NN=>NTN, iBW, rNOP, NE=>NTE, NOP=>globalNM, MDF, NOPP, s_mode, load=>dsol, NEL, NEM, NEV, NES, ths, bas
  ...
  integer(kind=ik) :: NB, LT_l, m2, m1, mDM, rad_ele, z_ele, iDM, ths2, ths3, ptn_g, ptn2_g, NSi,&
       prm, prm2, max_prs, s_frac, f_cut
  integer(kind=ik), allocatable:: piv(:) !piv(ths)
  real(kind=rk) :: tr
...
contains
  subroutine init_front(init,ptn,ptn2)
    ...
    integer(kind=ik) :: init, h, i, j, k, l, m, n, o, p, q, r, g, NNf(NN), last_e, last_n, flag1, l_i,&
         ele, ptn, rp, zp, ptn2, end_ele_d1, end_ele_dn, first_ele_di, e_move
    logical :: inc
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!USER_SPECIFIED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 !Last element in domain 1 and iDM (remember domain iDM is ordered from the lart element back)
       ...
       rad_ele = 2*NEL+NEV+NES

       end_ele_d1 = NEL**2 + NES*(NEL+NEV) !(NEL+NES)*(NEL+NEV)! 8*38!??
       end_ele_dn = NE - (end_ele_d1/rad_ele)*rad_ele + 1!??
WRITE(*,*) end_ele_d1, end_ele_dn
...
!!!!!!!!!!!!!!!!!!!!!!!!!USER_SPECIFIED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       ptn_g = ptn
       ptn2_g = ptn2
       if (ptn_g.eq.-3.and.ths.lt.4) then
          ptn_g = -2
       end if
       if (ptn_g.eq.-3) then
          ptn2_g = rad_ele/2
...
!
       ...
       allocate(loadc(NV),load_dum(NV),LT(NV,LT_l),IT(NV,3+LT_l),lt_i(iDM+1),dm_type(iDM),Etest(NE),&
            NODf(iDM+1,NV),NOA(NV),ele_list(NE+1,iDM),dm_list(iDM,2),lt_i2(2+1))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ORDERING FOR DUAL FRONT!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       if (iDM.eq.2) then !Dual domain standard order, and reversed standard
          ...
          do i = 1, NE, 1
             ...
             call exclude_check(ele_list(i,1),inc)
             if (inc) p = p + 1
          end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!USER_SPECIFIED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine determine_offsets()
    implicit none
    integer(kind=ik) :: i, j, k, ele, l, l_i, o, p
    logical :: inc
  ...
  subroutine multifront_single(L2res,t)
    ...
    if(j.ne.0) write(*,*) 'Small Pivots:', j
  ...
  subroutine multifront_dual2(L2res,t)
    ...
    use omp_lib
    implicit none
    real(kind=rk) :: front(iBW,iBW,iDM),  loadf(iBW,iDM), loadf_dum(iBW,iDM), L2res,&
         t0, t1, t2, t, t_diff, t_ele, t_i, t_d(iDM)
    integer(kind=ik) :: iPg(iBW,iDM), jPg(iBW,iDM), fs(iDM), e_n(4), i, j, k, l_i, id, s, e_move, o, p
    ...
    t_diff = t_d(2) - t_d(1)
    !t_diff = 1.0_rk
    if (ABS(t_d(1) - t_d(2)).gt.0.01_rk*(t_d(1)+t_d(2)-2.0_rk*t1)) then
       ! write(*,*) 'Load balancing'
       ! write(*,*) (t_d(1) - t_d(2)), 0.01_rk*(t_d(1)+t_d(2)-2.0_rk*t1)

       if (t_diff.gt.0.0_rk) then
          o = 1 !receives
          p = 2 !gives
       else
          o = 2
  ...
  subroutine multifront_dual(L2res,t)
    ...
    implicit none
    real(kind=rk) :: front(iBW,iBW,iDM),  loadf(iBW,iDM), loadf_dum(iBW,iDM), L2res,&
         t0, t1, t2, t, t_diff, t_ele
    integer(kind=ik) :: iPg(iBW,iDM), jPg(iBW,iDM), fs(iDM), e_n(4), i, j, k, l_i, id, o, p, e_move
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!PHASE 3.0!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine elim_stitch(front,loadf,loadf_dum,iPg,jPg,l_i,fs,id)
    ...
    do i = 1, fs, 1
       if (iPg(i).lt.0) then
          !write(*,*) 'Found fully summed in stitch'
          rvs = rvs + 1
          RSUM(rvs) = i
          iPg(i) = -iPg(i)
       end if
    ...
    call elim_summed(front,loadf,loadf_dum,iPg,jPg,rvs,cvs,RSUM,CSUM,l_i,fs,id,-1)
  ...
  Subroutine Front_solve(fs,front,loadf,loadf_dum,iPg,jPg,l_i,id,dm,mode,e_n)
    ...
    do
       ...
       call elim_summed(front,loadf,loadf_dum,iPg,jPg,rvs,cvs,RSUM,CSUM,l_i,fs,id,ele)
  ...
  subroutine elim_summed(front,loadf,loadf_dum,iPg,jPg,rvs,cvs,RSUM,CSUM,l_i,fs,id,ele)
    implicit none
    integer(kind=ik) :: fs, iPg(iBW),jPg(iBW)
    real(kind=rk) :: front(iBW,iBW), loadf(iBW), loadf_dum(iBW)
    integer(kind=ik) ::  i, j, k, id, ele,&
